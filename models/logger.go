...
func (l Logrus) OK(c echo.Context, status int, start time.Time) {
	l.Info(NewLoggerMessage(
		nil,
		status,
		c,
...
func (l Logrus) Error(err CustomError, c echo.Context, status int, start time.Time) {
	l.Warn(NewLoggerMessage(
		&err,
		status,
		c,
		time.Since(start).String(),
